...
namespace Downloader 
{
    ...
    public class Downloader : IDataErrorInfo, INotifyPropertyChanged
    {
        ...
        #region DisplayResults
        private bool _MenuItem_DisplayResults_Flag = false;
        public bool MenuItem_DisplayResults_Flag
        {
            get { return _MenuItem_DisplayResults_Flag; }
            set
            {
                if (_MenuItem_DisplayResults_Flag != value)
                {
                    _MenuItem_DisplayResults_Flag = value;
                    NotifyPropertyChanged("MenuItem_DisplayResults_Flag");
                }
            }
        }
        #endregion
        ...
        private void StartBatchDownload()
        {
            ...
            Task DownloadFilesLoopTask = Task.Factory.StartNew(() =>
            {
                ...
                //todo: add view of cached urls below main window?
                ...
                //todo: how to get downloader window to minimize when opening a tool window than pop back up on closure of popup window?
                //todo: add album and artist regex options?
                ...
                else
                {
                    ...
                    if (MenuItem_RenameFiles_Flag)
                    {
                        ...
                        {
                            RenameFile.RenameDirectory(DirectoryPath, displayResults: MenuItem_DisplayResults_Flag);
        ...
        private void AddUrlToQueue(string urlSubSection)
        {
            if (!UniqueUrls.Contains(urlSubSection))
            {
                ...
                else
                {
                    ...
                    lock (UrlsToRecurse)
                    {
                        UrlsToRecurse.Add(GetBaseUrl(OriginalUrl) + urlSubSection);
                    }
        ...
        private void ActuallyDownloadFile(WebClient client, string url)
        {
            ...
            client.DownloadFileCompleted += (sender, e) =>
            {
                ...
                if (e.Error != null)
                {
                   // MessageBox.Show("Error Occured Downloading: {0}", Error);
                }
                else if (e.Cancelled)
                {
