...
namespace Downloader24
{
    public class Downloader : IDataErrorInfo, INotifyPropertyChanged
    {
        ...
        /// </summary>
        private HashSet<string> UrlsToDownload = new HashSet<string>();
        /// <summary>
        /// hashset of tuples representing a url and whether it is a file or not
        /// </summary>
        private HashSet<string> UrlsToRecurse = new HashSet<string>();
        private Queue<string> RecurseUrlsQueue = new Queue<string>();
        ...
        private Task<string> DownLoadFileAsync(string url, bool recursive = false)
        {
            ...
            using (WebClient client = new WebClient())
            {
                ...
                {
                    //Handling the DownloadFileCompleted Event
                    ...
                    client.DownloadProgressChanged += (s, e) =>
                    {
                        ...
                        ////Setting filename
                        //mainWindow.CurrentDownloadLabel.Content = FileName;

                        ////Setting speed
                        //mainWindow.speedlabel.Content = string.Format("{0:##.##}", (e.BytesReceived / bytesToMBFactor / Downloader.Stopwatch.Elapsed.TotalSeconds).ToString("0.00"));

                        ////Setting amount to download
                        //mainWindow.downloadedlabel.Content = string.Format("{0:##.##}", e.TotalBytesToReceive / bytesToMBFactor);

                        ////Setting total Downloaded
                        //mainWindow.totaldownloadlabel.Content = string.Format("{0:##.##}", e.BytesReceived / bytesToMBFactor);
